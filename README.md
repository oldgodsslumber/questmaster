...
**Parties** are top-level Firestore documents keyed by their own invite code, so joining is a direct `get(parties/CODE)` — no query, no composite index. A crawler carries a `partyIds` array (migrated automatically from the old single `partyId`) and can be in several at once; the Feed blends them into one stream, tagged and filterable by party. **Friends** work the same keyed-lookup way: a public `crawlers/{uid}` card holds the shareable minimal profile and a `crawlerCodes/{CODE}` index resolves a friend code to a uid, so "add by code" is one read and never exposes anyone's private sheet. Posting "to friends" writes to a personal wall at `crawlers/{uid}/feed`, readable by anyone who has added that crawler (a follower); the Feed blends your followed friends' walls in alongside your parties, filterable to just **Friends**. Journal entries, manual notes, and statuses/achievements can all be shared to Friends or a party — statuses and achievements from a **Share** button both at creation and later from the list. The Party and Feed screens are the only place in the app that subscribes to live snapshots; everything else still follows the fetch-once rule.
